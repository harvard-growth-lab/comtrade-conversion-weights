...
for i in "${!START_YEARS[@]}"; do
    ...
    matlab -nosplash -nodesktop -batch "data_source='${DATA_SOURCE}'; start_year=${START_YEARS[i]}; end_year=${END_YEARS[i]}; groups=1:${MAX_GROUPS[i]}; tol=1e-20; diary('${LOGS_DIR}/matlab_output_${START_YEARS[i]}_${END_YEARS[i]}.log'); run('MAIN_Matlab_optimization_GL.m'); diary off" 2>&1 | tee -a "$LOG_FILE"
