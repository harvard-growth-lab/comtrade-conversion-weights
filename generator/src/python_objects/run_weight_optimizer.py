...
class MatlabProgramRunner(Base):
    ...
    def write_matlab_params(self) -> None:
        ...
        files = matrices_dir.glob(f"conversion.matrix.{self.data_source}.*.csv")
        ...
        for self.conversion_weight_pair in self.conversion_weights_pairs:
            ...
            if self.data_source == "comtrade":
                ...
                self.source_year = int(self.conversion_weight_pair['source_year'])
                self.target_year = int(self.conversion_weight_pair['target_year'])
        ...
        with open(output_file, "w") as f:
            ...
            f.write(f'DATA_SOURCE="{self.data_source}"\n')
    ...
    def extract_max_groups(self, filenames: list[Path]) -> dict:
        ...
        pattern = r"conversion\.matrix\.\w+\.start\.(\d+)\.end\.(\d+)\.group\.(\d+)\.csv"
